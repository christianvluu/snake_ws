...
def add_low_freq_noise(raw_data):
    white_noise = np.random.normal(0, 0.1, len(raw_data))
    hz = 20
    high_cut = 11 # hz
    low_cut = 1 # hz
    order = 5
    noise = butterworth_bandpass_filter(white_noise, low_cut, high_cut, hz, order)
    return noise


def add_current_spike(prev_pos, curr_pos, loops, status):
    ...
    if new_status != status[0]:
        ...
        noise_factor = np.random.normal(0.8, 0.3, len(prev_pos))
        loops[0] = 1
    elif (0 < loops & loops < 10): # add the spike for 0.1 seconds
        noise_factor = np.random.normal(0.8, 0.3, len(prev_pos))
        loops[0] += 1
    else:
        noise_factor = np.ones(len(prev_pos)) # don't add noise here
        loops[0] = 0
    status[0] = new_status
    return noise_factor
